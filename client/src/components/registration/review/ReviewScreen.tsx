...
export function ReviewScreen({ values, files, onEdit }: Props) {
  return <div className="space-y-4"><RegSectionTitle title="Review your application" subtitle="Check all details before submitting." />
    <ReviewSection title="Personal" stepId={1} onEdit={onEdit}><ReviewField label="Name" value={values.name} /><ReviewField label="Father" value={values.fatherName} /><ReviewField label="Mother" value={values.motherName} /><ReviewField label="Gender" value={values.gender} /></ReviewSection>
